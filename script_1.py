# Update the TV data with more accurate box dimensions from research
import json

# Updated TV data with more accurate box dimensions from actual product specifications
tv_data_updated = {
...
# Save updated data
with open('tv_fitment_data_final.json', 'w') as f:
    json.dump(tv_data_updated, f, indent=2)

print("Updated TV Fitment Tool Data:")
print(f"Total TV Brands: {len(tv_data_updated['tv_brands'])}")
print(f"Total TV Sizes: {len(tv_data_updated['tv_sizes'])}")
print(f"Total Car Models: {sum(len(models) for models in tv_data_updated['car_models'].values())}")

# Display the complete data structure for reference
print("\nFinal Data Structure Preview:")
print("TV Brands:", tv_data_updated['tv_brands'])
print("\nSample TV Sizes (55\", 65\", 75\"):")
for size in ['55', '65', '75']:
    print(f"{size}\": {tv_data_updated['tv_sizes'][size]}")